...
from beta.api.okex import OKEX_Sub_Spot_Api, OKEX_Contract_Api, okex_all_symbol_pairs, okex_all_contract_symbol, \
    okex_all_symbol_type, okex_all_contract_all_symbol_usd, okex_all_contract_type
...
class okexGateway(VtGateway):
    ...
    def connect(self):
        ...
        try:
            ...
            log.logContent = u'read connection config error, please check'
            self.onLog(log)
            return
        ...
        try:
            ...
            log = VtLogData()
            log.gatewayName = self.gatewayName
            log.logContent = u'connection config missing field, please check'
            self.onLog(log)
            return
        ...
        log = VtLogData()
        log.gatewayName = self.gatewayName
        log.logContent = u'Interface init success'
        self.onLog(log)

        self.writeLog(u'Interface init success')
    ...
    def isConnected(self):
        return self.spot_connected and self.futures_connected
    ...
    def subscribe(self, subscribeReq):
        ...
        try:
            ...
            self.onError(u'OkexGateway.subscribe exception, please check log')
    ...
    def sendOrder(self, orderReq):
        ...
        if ':' not in orderReq.symbol:
            if self.api_spot and self.spot_connected:
                ...
                self.writeError(u'spot interface not init/connect, cant call sendOrder')
                return ''
        else:
            if self.api_contract and self.futures_connected:
                return self.api_contract.futureSendOrder(orderReq)
            else:
                self.writeError(u'future interface not init/connect, cant call subscribe')
                return ''
    ...
    def qryAccount(self):
        ...
        if self.futures_connected == True:
            self.api_contract.fetchAllFullMarginUserInfo()
    ...
    def qryPosition(self):
        if self.futures_connected == True:
            self.api_contract.fetchAllFullMarginPositons()
    ...
    def initQuery(self):
        ...
        if self.qryEnabled:
            ...
            self.qryTrigger = 60  # 查询触发点 1 minutes
...
class Api_contract(OKEX_Contract_Api):
    ...
    def __init__(self, gateway):
        ...
        self.localNoQueue = []  # 未收到系统委托号的本地委托号队列
        ...
        if isinstance(ws_data, dict):
            if 'event' in ws_data:
                ...
                if ws_data['event'] == 'pong':
                    self.gateway.writeLog(u'ok contract heart beat response {}'.format(datetime.now()))
                    return
                else:
                    self.gateway.writeLog(u'other event:{}'.format(ws_data))
            else:
                self.gateway.writeLog(u'other data:{}'.format(ws_data))
    ...
    def reconnect(self):
        while not self.gateway.futures_connected:
            self.writeLog(u'okex Api_contract wait 10s and reconnect')
    ...
    def onClose(self, ws):
        ...
        self.writeLog(u'server connect break')
    ...
    def subscribeSingleSymbol(self, symbol, contract_type, leverage):
        ...
        if symbol in okex_all_contract_symbol:
            ...
            self.subscribeFutureKline(symbol, contract_type, '1min')
    ...
    def onOpen(self, ws):
        ...
        self.writeLog(u'server okex contract connect successfully')
        ...
        for symbol in okex_all_contract_symbol:
            ...
            for use_contract_type in okex_all_contract_type:
                ...
                self.channelSymbolMap["ok_sub_futureusd_%s_kline_%s_1min" % (symbol, use_contract_type)] = use_symbol_name
                ...
                contract.name = u'OKEX future %s_%s_%s' % (symbol, use_contract_type, self._use_leverage)
                ...
                contract.exchange = EXCHANGE_OKEX
                contract.vtSymbol = contract.symbol
                contract.name = u'OKEX future account rights %s' % (symbol)
                contract.size = 0.00001
                contract.priceTick = 0.00001
                contract.productClass = PRODUCT_FUTURES
                self.gateway.onContract(contract)
        ...
        self.login()
        # 连接后查询账户和委托数据
        self.fetchAllFullMarginUserInfo()
        self.fetchAllFullMarginPositons()
        ...
        self.subscribeSingleSymbol("btc", "quarter", "10")
        self.subscribeSingleSymbol("eth", "quarter", "10")
        self.subscribeSingleSymbol("eos", "quarter", "10")
        # self.subscribeSingleSymbol("ltc", "quarter", "10")
        # self.subscribeSingleSymbol("etc", "quarter", "10")
        # self.subscribeSingleSymbol("bch", "quarter", "10")
        # self.subscribeSingleSymbol("xrp", "quarter", "10")

    # ----------------------------------------------------------------------
    def fetchAllFullMarginUserInfo(self):
        self.onFutureUserInfo(self.fetchFutureUserInfo())

    # ----------------------------------------------------------------------
    def fetchAllFullMarginPositons(self):
        self.notifyFullMarginFuturePositionInfo("btc", "quarter")
        self.notifyFullMarginFuturePositionInfo("eth", "quarter")
        self.notifyFullMarginFuturePositionInfo("eos", "quarter")
        # self.notifyFullMarginFuturePositionInfo("ltc", "quarter")
        # self.notifyFullMarginFuturePositionInfo("etc", "quarter")
        # self.notifyFullMarginFuturePositionInfo("bch", "quarter")
        # self.notifyFullMarginFuturePositionInfo("xrp", "quarter")

    # ----------------------------------------------------------------------
    def notifyFullMarginFuturePositionInfo(self, symbol, type):
        self.writeLog("onFullMarginFuturePositionInfo for %s-%s:" % (symbol, type))
        fullMarginPosition = self.fetchFullMarginFuturePositionInfo(symbol, type)
        # self.writeLog("fetched full margin pos for %s-%s:" % (symbol, type))
        holdings = json.loads(fullMarginPosition)['holding'][0]
        # self.writeLog("future holding info: {}".format(str(holdings)))
        contractSymbol = holdings['symbol']

        if holdings['buy_amount'] > 0:
            pos = VtPositionData()
            pos.symbol = symbol + "_usdt"
            pos.exchange = EXCHANGE_OKEX
            pos.vtSymbol = contractSymbol + ":" + str(holdings["contract_type"]) + ":" + str(holdings["lever_rate"]) + "." + EXCHANGE_OKEX
            pos.direction = DIRECTION_LONG
            pos.position = float(holdings["buy_amount"])
            pos.frozen = pos.position - float(holdings['buy_available'])
            pos.price = float(holdings["buy_price_avg"])
            pos.vtPositionName = pos.vtSymbol + "." + pos.direction
            pos.positionProfit = float(holdings["buy_profit_real"])
            self.gateway.onPosition(pos)
        if holdings['sell_amount'] > 0:
            pos = VtPositionData()
            pos.symbol =  symbol + "_usdt"
            pos.exchange = EXCHANGE_OKEX
            pos.vtSymbol = contractSymbol + ":" + str(holdings["contract_type"]) + ":" + str(holdings["lever_rate"]) + "." + EXCHANGE_OKEX
            pos.direction = DIRECTION_SHORT
            pos.position = float(holdings["sell_amount"])
            pos.frozen = pos.position - float(holdings['sell_available'])
            pos.price = float(holdings["sell_price_avg"])
            pos.vtPositionName = pos.vtSymbol + "." + pos.direction
            pos.positionProfit = float(holdings["sell_profit_real"])
            self.gateway.onPosition(pos)
    ...
    def initCallback(self):
        ...
        for symbol in okex_all_contract_symbol:
            ...
            for use_contract_type in okex_all_contract_type:
                ...
                self.cbDict["ok_sub_futureusd_%s_kline_%s_1min" % (symbol, use_contract_type)] = self.onBar
...
h_this_week_usd'}
    ...
    def onBar(self, data):
        """
        on subscribe:
        {u'binary': 0, u'data': {u'result': True, u'channel': u'ok_sub_futureusd_xrp_kline_quarter_1min'}, u'channel': u'addChannel'}
        on Bar:
        {u'binary': 0, u'data': [[u'1529755740000', u'0.484', u'0.484', u'0.484', u'0.484', u'60.0', u'1239.669421487603']], u'channel': u'ok_sub_futureusd_xrp_kline_quarter_1min'}
        """
        try:
            #print data
            data_list = data["data"]
            ...
            bar = data_list[0]
            firstBar = False
            dt = datetime.fromtimestamp(int(bar[0][:-3]))
            if channel in self.barCache:
                barInCahe = self.barCache[channel]
                if (barInCahe.datetime == dt):
                    newBar = False
                else:
                    newBar = True
            else:
            ...
            if newBar:
                barData = VtBarData()
                barData.symbol = self.channelSymbolMap[channel] + '.' + EXCHANGE_OKEX
                barData.vtSymbol = barData.symbol
                barData.exchange = EXCHANGE_OKEX
                barData.gatewayName = self.gatewayName
            else:
                barData = barInCahe
            ...
            barData.volume = float(bar[5])  #number of future contracts instead of quantities

            barData.date = dt.strftime('%Y%m%d')
            barData.time = dt.strftime('%H:%M:%S.%f')
            barData.datetime = datetime.strptime(' '.join([barData.date, barData.time]), '%Y%m%d %H:%M:%S.%f')

            if newBar and not firstBar:
                self.gateway.onBar(barInCahe)

            self.barCache[channel] = barData
    ...
    def onFutureOrder(self, data):
        """委托信息查询回调"""
        self.writeLog("onFutureOrder:")
        try:
            rawData = data['data']
            if 'error_code' in rawData.keys():
                self.writeLog("onFutureOrder: error" + str(rawData))
                return

            orderId = str(rawData['order_id'])
            self.writeLog("onFutureOrder: " + orderId + ". info: " +  str(rawData) )
    ...
    def onFutureOrderCancel(self, data):
        """委托信息查询回调"""
        self.writeLog("onFutureOrderCancel:")
        if 'data' not in data:
            return
...
0247, u'bond': 0.04018001}]
    ...
    def onFutureUserInfo(self, data):
        ## TODO check when this call back will happen, check the logic
        try:
            self.writeLog("onFutureUserInfo.")
            info = json.loads(data)['info']
            # self.writeLog("future user info: {}".format(str(info)))
            for symbol in info.keys():
                ...
                if "account_rights" in u_inf.keys():
                    ...
                    account.accountID = symbol + "_usdt"
                    account.vtAccountID = account.accountID + ".future." + EXCHANGE_OKEX
                    account.balance = float(u_inf["account_rights"])
                    account.margin = float(u_inf['risk_rate'])
                    self.gateway.onAccount(account)
                else:
                    # 说明是逐仓返回
                    t_contracts = u_inf["contracts"]
                    t_balance = float(u_inf["risk_rate"])
                    t_rights = float(u_inf["account_rights"])
            ...
            print("Error reading future user info.")
            print ex
...
, u'deal_amount': 0.0}, u'channel': u'ok_sub_futureusd_trades'}
    ...
    def onFutureSubTrades(self, data):
        self.writeLog('ok_sub_futureusd_trades')

        """发单"""
        try:
            if "error_code" in data.keys():
                self.writeLog("On Future Sub trade: error code " + str(data[error_code]))

                return
            rawData = data["data"]
            """
            {
              u'orderid': 1163872103511040,
              u'contract_name': u'EOS0928',
              u'fee': 0.0,
              u'user_id': 8428542,
              u'contract_id': 201809280200053,
              u'price': 8.5,
              u'create_date_str': u'2018-07-25 13:07:58',
              u'amount': 1.0,
              u'status': 0,
              u'system_type': 0,
              u'unit_amount': 10.0,
              ...
              u'create_date': 1532495278926,
              u'lever_rate': 10.0,
              u'type': 1,
              u'deal_amount': 0.0
            }
            """
            self.writeLog("On Future Sub trade: " + str(data))
            ...
            if not localNo:
                for req  in self.localNoQueue:
                    localNo = req.getLocalId()
                    pricel = req.getPrice()
                    vt_symboll = req.getSymbol()
                    symboll, ct_type, _ = vt_symboll.split(":")
                    #sidel = req.getSide()
                    voll = req.getVolume()

                    if abs(float(pricel) -  float(rawData['price'])) < 0.00001 \
                        and symboll.split("_")[0] == (self.contract_name_dict[rawData["contract_name"]])["symbol"]  \
                        and ct_type == use_contract_type \
                        and float(voll) - float(rawData['amount']) < 0.001 :
                        self.localNoDict[localNo] = orderId
                        self.orderIdDict[orderId] = localNo
                        self.localNoQueue.remove(req)
                        break
            ...
            if orderId not in self.orderDict:
                ...
                order.symbol = '.'.join(
                    [symbol + ":" + use_contract_type + ":" + str(self._use_leverage), EXCHANGE_OKEX])
            ...
            self.writeLog("call gate way onOrder:\n"
                          " locId:{}, status:{}, symbol:{}, vol:{}, tradVol:{}, price:{}, direction:{}".format(
                order.orderID, order.status, order.vtSymbol, order.totalVolume, order.tradedVolume, order.price, order.direction
            ))
...
utureusd_positions'}
    ...
    def onFutureSubPositions(self, data):
        ...
        # 这个逐仓才是实际的仓位 ！！
        try:
            self.writeLog("on Future Sub Position")
            rawData = data["data"]
            ...
            for inf in positions:
                if 'fixmargin' in inf.keys():
                    ...
                    """
                    {
                      u'positions': [
                        {
                          u'contract_name': u'XRP0928',
                          u'realized': -0.68387097,
                          u'contract_id': 201809280150047,
                          u'position_id': 962381038240768,
                          u'bondfreez': 7.26991304,
                          u'avgprice': 0.481,
                          u'eveningup': 0.0,
                          u'hold_amount': 0.0,
                          u'costprice': 0.481,
                        ...
                        },
                        {
                          ...
                          u'position_id': 962381038240768,
                          u'bondfreez': 7.26991304,
                          u'avgprice': 0.46483515,
                          u'eveningup': 107.0,
                          u'hold_amount': 107.0,
                          u'costprice': 0.46500932,
                          u'position': 2,
                          u'margin': 0.0
                        }
                      ],
                    ...
                    """
                    self.writeLog("!!!!!  amazing ok_sub_futureusd_positions: %s" % data)
        except Exception, ex:
    ...
    def futureSendOrder(self, req):
        ...
        self.writeLog("okex contract -- send order {symbol} {side} {volume} @ {price} ".format(
            symbol=symbol, side=type_, volume=req.volume, price=req.price) )
...
class Api_Spot(OKEX_Sub_Spot_Api):
    ...
    def __init__(self, gateway):
        ...
        self.registerSymbolPairArray = {'btc_usdt', 'eos_usdt', 'eth_usdt'}
...
, u'size': u'0.01000000'}}
    ...
    def onMessage(self, ws, evt):
        ...
        if isinstance(ws_data, dict):
            if 'event' in ws_data:
                # 处理心跳得
                if ws_data['event'] == 'pong':
                    self.gateway.writeLog(u'ok spot heart beat response {}'.format(datetime.now()))
                    return
                else:
                    self.gateway.writeLog(u'other event:{}'.format(ws_data))
            else:
    ...
    def reconnect(self):
        while not self.gateway.spot_connected:
            self.writeLog(u'okex api_spot wait 10s and re-connect')
    ...
    def onClose(self, ws):
        ...
        self.writeLog(u'server connection break')
    ...
    def onOpen(self, ws):
        ...
        self.writeLog(u'OKEX spot server connect successfully')
        ...
        for symbol in okex_all_symbol_pairs:
            ...
            contract.name = u'OKEX spot %s' % symbol
            contract.size = 0.00001
            contract.priceTick = 0.00001
...
: u'0', u'eos': u'0', u'etc': u'7.9909635', u'act': u'0', u'eth': u'0', u'ltc': u'0', u'bcs': u'0'}}
    ...
    def onSpotUserInfo(self, data):
        """现货账户资金推送"""
        rawData = data['data']
        # self.writeLog("spot usder info: {}".format(str(rawData)))
...
nel': u'ok_sub_spot_etc_usdt_order'}
    ...
    def onSpotSubOrder(self, data):
        ...
        if order.status in [STATUS_CANCELLED, STATUS_PARTTRADED, STATUS_ALLTRADED]:
            ...
            if order.status == STATUS_CANCELLED:
                self.gateway.writeLog("order Canceled : order.totalVolume %s  order.tradedVolume %s order.price %s" % (
                str(order.totalVolume), str(order.tradedVolume), str(order.price)))
            elif order.status == STATUS_ALLTRADED:
                self.gateway.writeLog("order alltraded : order.totalVolume %s  order.tradedVolume %s order.price %s" % (
                str(order.totalVolume), str(order.tradedVolume), str(order.price)))
    ...
    def autoFixMatch(self, to_compare_order, t_localID):
        self.gateway.writeLog(
            "self.cacheSendLocalOrder : %s , t_localID %s " % (str(self.cacheSendLocalOrder), t_localID))
        for localID in self.cacheSendLocalOrder:
            ...
            if from_order != None:
                ...
                if from_order["direction"] == to_compare_order.direction and from_order[
                    "offset"] == to_compare_order.offset and abs(
                        float(from_order["price"]) - float(to_compare_order.price)) < 0.01 * float(
                        to_compare_order.price) and abs(
                        float(from_order["volume"]) - float(to_compare_order.totalVolume)) < 0.01 * float(
                        to_compare_order.totalVolume):
                    self.gateway.writeLog(
                        "autoFixMatch compare order now! t_localID %s , localID %s" % (str(t_localID), str(localID)))
                    t_localID = localID
                    break
            ...
            try:
                ...
                if orderId not in self.orderDict:
                    ...
                    if orderId not in self.orderIdDict.keys():
                        ...
                        self.gateway.writeLog(
                            " onSpotOrderInfo occure new order, localNo %s, orderId %s , order.vtSymbol %s , order.price %s d:%s" % (
                            str(localNo), str(orderId), str(order.vtSymbol), str(order.price), str(d)))
                ...
                if order.status == STATUS_CANCELLED:
                    self.gateway.writeLog(
                        " order is canceld !  orderId:%s , vtOrderID:%s" % (str(orderId), str(order.vtOrderID)))
    ...
    def onSpotOrder(self, data):
        ...
        if 'error_code' in data:
            ...
            # todo:增加错误id得解释映射获取
            msg = OKEX_Error_Info.get(error_id, u'SpotApi.onSpotOrder return error')
            ...
            if detail_info_dict != None:
                ...
                self.gateway.writeError(u"onSpotOrder, rejection order id info cant be found localNo " % (str(localNo)), 0)
            ...
            if orderId is None:
                self.gateway.writeError(u'SpotApi.onSpotOrder order returning, no orderid')
    ...
    def onSpotCancelOrder(self, data):
        ...
        if 'error_code' in data["data"].keys():
            error_id = str(data.get('error_code', 0))
            # todo:增加错误id得解释映射获取
            msg = OKEX_Error_Info.get(error_id, u'SpotApi.onSpotCancelOrder cancel error')
            self.gateway.writeError(msg, error_id=error_id)
            return
        try:
            rawData = data['data']
            ...
            self.gateway.writeLog("onSpotCancelOrder 1 : order.totalVolume %s  order.tradedVolume %s order.price %s" % (
            str(order.totalVolume), str(order.tradedVolume), str(order.price)))
    ...
    def spotSendOrder(self, req):
        """发单"""
        self.gateway.writeLog(
            "spotSendOrder %s , %s , %s , %s" % (req.vtSymbol, req.direction, str(req.price), str(req.volume)))
        ...
        self.localNoInfo[localID] = {"symbol": symbol, "vtSymbol": req.vtSymbol, "price": req.price,
                                     "volume": req.volume, "direction": req.direction, "offset": offset,
                                     "orderTime": "spotSendOrder:" + datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
        req.setLocalId(self.localNo)
        self.localNoQueue.append(req)
